	...
	<div class="col-xs-my-40">
		<form action="<%=path%>/shoppingproductses" method="get"
			...
			<div class="j-right">
				...
				<div class="col-xs-my-1">&nbsp;</div>
				<div class="col-xs-my-4 j-btn-act">
					<a href="<%=path%>/shoppingproductses?page=1"
						...
						<tbody>
							<c:forEach items="${shoppingproductses }" var="shoppingproduct">
								<tr class="odd">
									...
									<td class="j-odd2">￥${shoppingproduct.price}</td>
									<c:if
									...
									<td class="j-odd2"><fmt:formatDate value="${shoppingproduct.onSaleDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
									
									<td class="j-odd2"><c:if test="${shoppingproduct.isExpect==true}" >是</c:if><c:if test="${shoppingproduct.isExpect==false}" >否</c:if></td>
									
									<c:if test="${shoppingproduct.deleteStatus != -1}">
									<td><a href="<%=path %>/shoppingproductses/${shoppingproduct.id}?form" class="j-odd4">编辑</a> <a class="j-odd5"
										onclick="updateOnSale(${shoppingproduct.id})">下架</a></td>
							      	</c:if>
							      	<c:if test="${shoppingproduct.deleteStatus == -1}">
							      		<c:if test="${shoppingproduct.onSale ==0}">
									<td>商品正在参加促销活动</td>
							      	</c:if>
							       	<c:if test="${shoppingproduct.onSale !=0}">
									<td><a href="<%=path %>/shoppingproductses/${shoppingproduct.id}?form" class="j-odd4">编辑</a> <a class="j-odd5"

...
<div>
					...
					<form action="<%=path %>/shopfreighttemplates" method="post" id="form">
						...
						<div class="col-xs-12 j-register-top">
							...
							<div class="col-xs-8 form-group">
								<input type="text"  maxlength="20" name="name" datatype="*" value="${shopFreightTemplate.name }" class="col-xs-my-40 form-control" placeholder="请输入模板名称">
						...
						<div class="col-xs-my-48">
							...
							<div class="col-xs-my-44" style="background: #F8F8F8; margin-top: 50px;">
								<div class="col-xs-my-48 j-register-top">
									...
									<input type="hidden" name="shopFreightTemplateDetails[0].shopFreightTemplateDetailCities[0].provinces" value=""/>
									<div class="col-xs-my-2">&nbsp;</div>
								</div>
								<div class="col-xs-my-48 j-register-top">
									...
									<table class="table table-striped table-bordered table-hover dataTable no-footer" id="tableIds">
										<tr id="trs">
											...
											<tr id="trs${statu.index }">
												...
												<div class="col-xs-5"><label class="form-group fl checkbox"><input name="shopFreightFreeShipping[${statu.index }].count" type="radio" value="0" class="i"  checked />满<input name="shopFreightFreeShipping[${statu.index }].numbers" type="text" value="${shopFreightTemplate.shopFreightFreeShipping[statu.index].numbers}" class="input" />kg包邮</label></div>'
					                                  <div class="col-xs-5"><label class="form-group fl checkbox"><input name="shopFreightFreeShipping[${statu.index }].count" type="radio"   value="1"  class="i"  />满<input name="shopFreightFreeShipping[${statu.index }].freight" type="text" value="${shopFreightTemplate.shopFreightFreeShipping[statu.index].freight}"  class="input" />元包邮 </label></div>
					                                </c:if>
					                                 	<c:if test="${shopFreightFreeShipping.count==1 }">	
												<div class="col-xs-5"><label class="form-group fl checkbox"><input name="shopFreightFreeShipping[${statu.index }].count" type="radio" value="0" class="i"  />满<input name="shopFreightFreeShipping[${statu.index }].numbers" type="text" value="${shopFreightTemplate.shopFreightFreeShipping[statu.index].numbers}"  class="input" />kg包邮</label></div>'
					                                  <div class="col-xs-5"><label class="form-group fl checkbox"><input name="shopFreightFreeShipping[${statu.index }].count" type="radio"  class="i"   value="1"  checked/>满<input name="shopFreightFreeShipping[${statu.index }].freight" type="text" value="${shopFreightTemplate.shopFreightFreeShipping[statu.index].freight}"  class="input" />元包邮 </label></div>
					                                </c:if>
					                               </td>
	...
	$("#Preservation1").on('click',function(){
		...
		if($('#addTableId tr').length > 1){
			...
			if($('#addTableId tr').eq(1).find('td').eq(0).text() == '添加城市'){
				alert('请添加城市')
				return false
			}
			}
...
// 		指定运费 指定条件包邮
		...
		function showCity(){
			...
			$("#tableIds").append('<tr id="trss'+trid+'">'
					+'<td id="tage'+trid+'" class="o_border_right font_size6"><a id="city" onclick="addPostage('+trid+')">添加城市</a></td>'
					+'<td class="o_border_right font_size6"><div class="col-xs-5"><label class="form-group fl checkbox"><input name="shopFreightFreeShipping['+trid+'].count" checked="checked" type="radio" value="0" class="i"   />满<input name="shopFreightFreeShipping['+trid+'].numbers" type="text" value="" class="input"  />kg包邮</label></div>'
					+'<div class="col-xs-5"><label class="form-group fl checkbox"><input name="shopFreightFreeShipping['+trid+'].count" type="radio" class="i"   value="1" />满<input name="shopFreightFreeShipping['+trid+'].freight" type="text" value=""  class="input" />元包邮 </label></div></td>'
			...
			art.dialog.open('<%=path %>/shopfreighttemplates?city',{id:'send',title:'选择区域',width:650,height:490,lock:true, background:'#000000',
				close: function() {  
					...
					$(chk_value).each(function(index){
						...
						$("#trss"+id).append('<input type="hidden" class="llys" name="shopFreightFreeShipping['+id+'].shopFreightFreeShippingCity['+index+'].provinces" value="'+this+'"/>');
					});
